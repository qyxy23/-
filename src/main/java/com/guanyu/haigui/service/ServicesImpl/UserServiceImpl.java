...
import com.guanyu.haigui.utils.*;
...
public class UserServiceImpl implements UserService {
    ...
    @Resource
    private com.guanyu.haigui.service.WechatBindService wechatBindService;
    /**
     * 上传用户头像（核心方法）
     *
    ...
    public String updateUsername(String username) {
        String trimmed = UsernameUtils.validateForRegister(username);
        Long userId = BaseContext.getCurrentId();
        UserInfo current = userInfoRepository.findById(userId)
                .orElseThrow(() -> new BusinessException(404, "用户不存在"));
        if (StringUtils.hasText(current.getPassword())) {
            throw new BusinessException(403, "App 账号不支持修改用户名");
        }
        if (!UsernameUtils.isAutoGeneratedWechatUsername(current.getUsername())) {
            if (trimmed.equals(current.getUsername())) {
                return "用户名设置成功";
            }
            throw new BusinessException(403, "用户名已设置，不可修改");
        }
        if (trimmed.equals(current.getUsername())) {
            return "用户名设置成功";
        }
        UserInfo existing = UserDetailsMapper.selectUserInfoByUsername(trimmed);
        if (existing != null && !existing.getUserId().equals(userId)) {
            throw new UserAlreadyExistsException(trimmed);
        }
        UserDetailsMapper.updateUserUsername(trimmed, userId);
        return "用户名设置成功";
    }
    ...
    public UserInfoVO getUserInfo(Long userId) {
        ...
        UserInfo userInfo = userInfoRepository.findById(userId)
                .orElseThrow(() -> new IllegalArgumentException("用户不存在"));
        boolean wechatBound = wechatBindService.isWechatBound(userInfo);
        return UserInfoVO.from(userInfo, authorities, wechatBound);
    }
